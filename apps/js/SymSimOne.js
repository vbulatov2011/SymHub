...
const IMG_RUN = "images/ui/btn_play.svg";
const IMG_STOP = "images/ui/btn_pause.svg";
const IMG_RECORDING_STOP = "images/ui/btn_record_stop.svg";
const IMG_RECORDING_START = "images/ui/btn_record_start.svg";
const CUR_MOVE = 'url(images/ui/cur_move3.svg) 15 15';
const CUR_PICK = 'url(images/ui/cur_pick2.svg) 1 31';
const CUR_DRAW = 'url(images/ui/cur_draw2.svg) 1 31';
...
function SymSimOne(options) {
    ...
    */
    ...
    function makeToolbox() {
        ...
        let btn = {};
        const IMG = 'images/ui/';
        btn.tools = createImgBtn({title:'options',         src:IMG+'btn_options.svg',action:toggleGUI});        
        btn.run   = createImgBtn({title:'play',            src:IMG+'btn_play.svg',action:onToggleRun});
        btn.init  = createImgBtn({title:'initialize',      src:IMG+'btn_restart.svg',action:onInitSimulation});
        btn.recording  = createImgBtn({title:'recording',  src:IMG+'btn_record_start.svg',action:onToggleRecording});
        btn.download   = createImgBtn({title:'save image', src:IMG+'btn_download.svg',action:onSaveImage});
        btn.full_screen  = createImgBtn({title:'full screen',src:IMG+'btn_full_screen.svg',action:onFullScreen});
        btn.move       = createImgBtn({title:'move',src:IMG+'btn_move2.svg',action:()=>setTool('move')});
        btn.draw       = createImgBtn({title:'draw',src:IMG+'btn_draw.svg',action:()=>setTool('draw')});
        btn.pick       = createImgBtn({title:'pick',src:IMG+'btn_pick.svg',action:()=>setTool('pick')});
        let space       = createImgBtn({src:IMG+'btn_vbar.svg'});
